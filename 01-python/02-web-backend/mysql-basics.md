...
可以先把 [`MySQL`](./mysql-basics.md) 理解成：
...
这类数据都更适合放在 [`MySQL`](./mysql-basics.md) 里。
...
## [`Session`](./mysql-basics.md) 到底是什么

你可以先把 [`Session`](./mysql-basics.md) 理解成：
...
可以拆成这样理解：
- `name`：字段名
- [`Column`](./mysql-basics.md)：声明一列数据库字段
