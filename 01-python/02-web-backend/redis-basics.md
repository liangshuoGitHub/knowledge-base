...
可以先把 [`Redis`](./redis-basics.md) 理解成：

**后端里专门放高频临时数据的高速缓存仓库。**

它和 [`MySQL`](./mysql-basics.md) 最大的直觉差异可以先这么记：
- [`MySQL`](./mysql-basics.md)：更像长期仓库
- [`Redis`](./redis-basics.md)：更像高速临时仓库
...
所以项目里既有 [`MySQL`](./mysql-basics.md)，也有 [`Redis`](./redis-basics.md)，其实非常正常。
