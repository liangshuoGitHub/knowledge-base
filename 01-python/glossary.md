...
- 为什么会出现在这里：`yy-auth` 本质上就是一个接口服务，要靠它来组织路由、请求、返回和依赖注入
- 当前可以怎么记：**快速写 API 的框架**
...
- 为什么会出现在这里：写完 `FastAPI` 还得有人把服务跑起来，本地开发时常用它
...
- 为什么会出现在这里：`yy-auth` 这种企业服务不是只给本地玩，正式部署时会更偏向用它来托管服务进程
- 当前可以怎么记：**线上更稳的启动方式**
...
- 为什么会出现在这里：`yy-auth` 要频繁操作用户、权限、组织等数据，需要更规范地访问数据库
...
- 为什么会出现在这里：上层就算用了 `SQLAlchemy`，底层也得有人真正去连 MySQL
...
- 常见读法：麦 sequel
- 它是干什么的：常见的关系型数据库
- 为什么会出现在这里：像 `yy-auth` 这种认证与权限服务，需要长期保存用户、角色、权限、组织等结构化数据
...
- 为什么会出现在这里：在 `yy-auth` 里，它会先挂到 `request.state.db`，再传进 `UserService.__init__()`，最后变成 `self.session`
...
- 为什么会出现在这里：像 `ProviderSSO` 这种类，本质上就是把表结构映射成 Python 类
...
- 常见读法：卡拉姆 / 科伦
- 它是干什么的：ORM 里声明字段的方式
- 为什么会出现在这里：像 `name = Column(String(30), unique=True, comment='标识')` 这种写法，就是在定义表里的一列
...
- 常见读法：纳科斯
- 它是干什么的：配置中心 / 服务治理平台
- 为什么会出现在这里：`yy-auth` 这类企业服务往往不把关键配置写死在本地，而是从统一配置中心拉
- 当前可以怎么记：**公司统一放配置的地方**
...
- 常见读法：J W T
- 它是干什么的：一种常见的身份令牌机制
- 为什么会出现在这里：`yy-auth` 要处理登录态，后端需要验证请求是不是合法用户发来的
...
- 常见读法：I A M
- 它是干什么的：身份与权限管理
- 为什么会出现在这里：`yy-auth` 不只是登录服务，还涉及角色、权限、接口访问控制
- 当前可以怎么记：**谁能干什么的总管系统**
...
- 为什么会出现在这里：像 `UserService.__init__()` 这种写法，就是对象创建后用来设置初始属性的地方
...
- 为什么会出现在这里：像 `self.login_user = None` 这种写法，表示给当前实例对象设置属性，而不是给 [`__init__`](./01-basics/common-commands.md) 这个方法设置属性
