...
### 5. 代码示例与源码链接要分层使用
在知识库 Markdown 中，代码块、行内代码、源码链接分别承担不同职责，不能混用。

#### 示例代码必须用代码块的场景
- 连续 2 行及以上、需要让读者整体理解结构的代码
- 用于前后端类比的示例代码
- 用于解释语法、对象关系、调用流程的示例代码

这类内容必须使用 fenced code block，例如 `python`、`ts`、`bash`。
...
例如：`self`、`__init__`、`self.login_user = None`。
...
如果只是概念解释、伪代码、类比代码或讲解性示例，默认使用普通代码块或行内代码，不要加源码链接。
